...
def writeBatchHeader(fout, filename):
    ...
    <tr><td>NCycles</td><td>Noise</td><td>Shake</td><td>TrialNumber</td><td>SlidingWindow</td>
        <td>PyramidLevel</td><td>Alpha</td><td>Weighted</td><td>Percentile</td><td>CircError</td><td>KendallTau</td></tr>   
    """.format(filename))

def writeBatchHeaderCSV(fout, filename):
    fout.write("NCycles,Noise,Shake,TrialNumber,SlidingWindow,PyramidLevel,Alpha,Weighted,CircError,KendallTau")

def doBatchTests(filename, fout, batchidx = -1):
    idx = 0
    noise = 0
    for NCycles in [3, 5, 10, 15, 20, 25, 30, 40, 50]:
        for shake in [0, 10, 20, 30, 40, 50]:
