...
Our approach is geometric: we treat each video frame as a point in a high-dimensional Euclidean space, and analyze the {\em sliding window embedding} formed by this sequence of points. % MSB: would be nice to have a concise description of what this is, but that is tricky to write up. Alternatively, we could also state why this embedding is useful for 1-cycle estimation
...
We show quantitative results that highlight the robustness of our technique to noise and camera shake, and show qualitative results of single-cycle motion synthesis across a wide variety of scenarios.
